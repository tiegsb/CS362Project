...
test file, some of them are printf() statement help me see the output result. However I got 100% average branch statement which the test execute all the neccessary statement to pass the test. 
...
executes all the statement in the function that I test, and since I execute them all, if the tests passes that means it passes because there is no place for bug to hide.
...
the test that cover all the functions in the dominion file. This problem could improve as I write more test that cover other functions until I write tests that cover all the functions in dominion.c file, I believe I could achieve coverage of 95% or more just like my function and test coverage.
...
endTurn(), isGameOver(), scoreFor(), getWinner(), drawCard(), cardEffect(), discardCard(), gainCard(), updateCoins(), CEsteward(). I need to write test for these functions. 
	The coverage statement is helpful when the test fail but we do not cover all the part of the test, and may be the missing part in the test 
is the one that trigger the bug. 
...
to go back and find and fix some other bugs that fail that test as well. I find this difficult to write test for specific bug and only for that bug. The way I design my test is, I write a test that cover a whole function, to see whether or not the function has a bug or not. If the function has some bugs, I write a smaller test for large section in the function such as for loop or while loop, to see where the bug hidden. Finally I check individual single statement to find that specific bug. Basically I write test to search the bug in big area, then reduce the size of it smaller and smaller until I find the bug. 
...
smithy() function, the bug hide in the discardCard() function that I call to discard the smithy card after I play. I never thought there is bug in discardCard() function, because I assume that it is working properly. The lesson I learn is that calling other functions that we haven't test yet are not 100% working, there could be bugs in them as well.
