...
would cause a 3rd treasure card to be drawn. In the if(state->deckCount[currentPlayer]<1) was
changed to if(state->deckCount[currentPlayer]<=1) which would shuffle discard pile even when deck not empty.
